...
export class FileUploadValidationInterceptor implements NestInterceptor {
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    const contentType = req.headers["content-type"].split(
      APP_CONSTANTS.CHAR.SEMI_COLON
    )[0];

    contentType != null
      ? this.headerValidation(contentType)
      : this.throwError(contentType);
    return next.handle();
  }

  headerValidation(contentType) {
    return APP_CONSTANTS.REGEX.MULTIPART_CONTENT_TYPE.test(contentType)
      ? true
      : this.throwError(contentType);
  }
